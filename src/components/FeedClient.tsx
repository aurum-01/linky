...
export default function FeedClient({ initialVideos, initialToken, prefs }: Props) {
  const { videos, token, isLoadingMore, hasMore, loadMore } = useFeed({
    initialVideos,
    initialToken,
    prefs,
  });
  ...
  const feedItems = buildFeedItems(videos);
  ...
  const handleScroll = useCallback(() => {
    ...
    if (videos.length - raw <= 5 && !isLoadingMore && hasMore) {
      loadMore();
    }
  ...
  const activeVideo = feedItems[activeIndex]?.video;
  const dotStart = Math.max(0, activeIndex - 4);
  const dotEnd   = Math.min(feedItems.length - 1, dotStart + 8);

  return (
    /*
      Key layout fix: the outer wrapper fills the viewport but does NOT
      hide overflow globally — only the feed column clips its scroll.
      This is what was making the page unscrollable before.
    */
    <div className="flex items-stretch justify-center w-full h-[100dvh] bg-zinc-950">
      ...
      <aside className="hidden lg:flex flex-col items-end justify-center pr-8 w-60 shrink-0 gap-5 border-r border-white/5">
        <div className="text-right space-y-0.5">
          ...
          <p className="text-[10px] text-white/30 uppercase tracking-widest">For you</p>
        </div>
        <div className="h-px w-16 bg-white/10" />
        <div className="flex flex-wrap gap-1.5 justify-end">
          {prefs.niches.map((slug) => (
            <span key={slug} className="text-white/40 text-[11px] bg-white/5 border border-white/10 px-2 py-0.5 rounded-full capitalize">
              {slug.replace(/-/g, " ")}
            </span>
          ))}
        </div>
      ...
      {/* Feed column — this is the ONLY element that scrolls */}
      <div
        className="relative shrink-0 border-x border-white/5"
        ...
        <div className="absolute right-2 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-1.5 pointer-events-none">
          {Array.from({ length: dotEnd - dotStart + 1 }, (_, i) => {
            ...
            return (
              <div
                key={ri}
                className={`rounded-full transition-all duration-300 ${ri === activeIndex ? "w-[5px] h-5 bg-white" : "w-[3px] h-[3px] bg-white/20"}`}
              />
            );
          })}
        </div>
      ...
      <aside className="hidden lg:flex flex-col items-start justify-center pl-8 w-60 shrink-0 gap-4 border-l border-white/5">
        <div className="bg-white/[0.04] border border-white/8 rounded-2xl p-4 w-52 space-y-2.5">
          <p className="text-white/30 text-[10px] uppercase tracking-widest font-medium">Now playing</p>
          {activeVideo ? (
            <>
              <p className="text-white text-sm font-semibold line-clamp-2 leading-snug">{activeVideo.title}</p>
              <p className="text-white/40 text-xs">@{activeVideo.author}</p>
              {activeVideo.score > 0 && (
                <div className="flex items-center gap-1.5 pt-0.5">
                  <div className="h-[3px] bg-white/10 rounded-full flex-1">
                    <div
                      className="h-full bg-gradient-to-r from-violet-500 to-pink-500 rounded-full"
                      style={{ width: `${Math.min(100, activeVideo.score / 5)}%` }}
                    />
                  </div>
        ...
        <div className="bg-white/[0.04] border border-white/8 rounded-2xl p-4 w-52 space-y-2">
          <p className="text-white/30 text-[10px] uppercase tracking-widest font-medium">Up next</p>
          {feedItems
            .slice(activeIndex + 1, activeIndex + 5)
            .filter((item) => item.type === "video")
            .slice(0, 3)
            .map((item, i) => (
              <p key={item.key} className="text-white/40 text-xs truncate">
                {i + 1}. {item.video!.title}
              </p>
            ))}
        </div>

        <div className="bg-white/[0.04] border border-white/8 rounded-2xl p-4 w-52 space-y-2">
          <p className="text-white/30 text-[10px] uppercase tracking-widest font-medium">Advertise</p>
          <p className="text-white/30 text-xs leading-relaxed">Place your brand in front of millions.</p>
          <button className="text-white/50 text-[11px] border border-white/10 px-3 py-1 rounded-full hover:border-white/30 hover:text-white/80 transition">
            Get in touch →
          </button>
        </div>
      </aside>
